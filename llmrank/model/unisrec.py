...
class UniSRec(SASRec):
    ...
    def predict_on_subsets(self, interaction, idxs):
        candidate_item_emb = self.moe_adaptor(self.plm_embedding(torch.LongTensor(idxs).to(self.device))).to(self.device)
        if self.train_stage == 'transductive_ft':
            candidate_item_emb = candidate_item_emb + self.item_embedding(torch.LongTensor(idxs).to(self.device))
        item_seq = interaction[self.ITEM_SEQ]
        item_seq_len = interaction[self.ITEM_SEQ_LEN]
        item_emb_list = self.moe_adaptor(self.plm_embedding(item_seq))
        print(item_seq_len.cpu().numpy().mean())
        seq_output = self.forward(item_seq, item_emb_list, item_seq_len)
        seq_output = F.normalize(seq_output,dim=-1)
        candidate_item_emb = F.normalize(candidate_item_emb,dim=-1)
